...
def add_to_db(title, content, author, lang, password):
    ...
    paste_info = Bin(title=title, hash=hash, content=content, lang=lang,
                     key=key, author=author, time=time, password=password)
    db.session.add(paste_info)
    db.session.commit()
    db.session.refresh(paste_info)
    paste_info = {"title": title, "hash": hash, "content": content,
                  "lang": lang, "key": key, "author": author, "password": password}
    return paste_info


def welcome_mail(username):
    user = User.query.filter_by(username=username).first()
    welcome_mail = Message(
        'Welcome to NoobPaste', sender='noobsciencesmtp@gmail.com', recipients=[user.email])
...
def email_send(name, email, content):
    email_send = Message(f'✉ {name} sent you a new mail!',
                         sender='noobsciencesmtp@gmail.com', recipients=['noobscience123@gmail.com'])
    email_send.html = '<div style="text-align: center;"><style>h1{font-family: Calibri, sans-serif;font-size: 42px;}p{font-family: sans-serif;font-weight: 300;font-size: 18px;}h2{font-family: sans-serif;font-size: 28px;}a{text-decoration: none;transition-delay: 50ms;transition-duration: 1s;}</style><img src="https://github.com/newtoallofthis123/Assets/raw/main/Image/noobscience.png" style="width: 54px; height: 54px;" alt="NoobPaste Image"><h1>' + \
        name + ' sent you a mail!</h1><p>' + content + \
        '</p><h2>This was sent by <a href="mailto:' + \
        email + '">' + email + '</a></h2></div>'
...
def edit_db(author, title, password, content, language, hash):
    ...
    paste_info_content = {"title": title, "hash": hash, "content": encrypted_content,
                          "lang": language, "key": paste_content.key, "author": author, "password": password}
...
def send_news_letter(title, content):
    ...
    email_send = Message(
        f'{title}', sender='noobsciencesmtp@gmail.com', recipients=mail_list)
...
def shortpaw(url):
    data = {"url": url}
    shortpaw_page = json.loads(requests.post(
        f'https://shortpaw.herokuapp.com/api', params=data).content)
...
def qr_code_engine(url):
    ...
    qr_code.svg('paste/static/assets/images/qr_code.svg',
                background="white", scale=8)
